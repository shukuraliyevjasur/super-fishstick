...
| 2 | ~~`removeOnFail: { age: 300 }` erases the counter the CEO-review alert would read~~ | ~~**Base the threshold on `getWorkerAlerts()`.**~~ **Done 2026-08-07.** `checkWorkerAlerts()` in `health-report.ts` reads the durable store; 3+ failures in 10 min triggers an email. |
